...
class AgendaInterface:
    @classmethod
    def list_student_event(
        cls,
        agent: TeacherAgent,
        date_range_start: datetime | None = None,
        date_range_stop: datetime | None = None,
    ) -> QuerySet[Event]:
        event_qs = Event.objects.all().filter(students=agent.id)
    ...
    @classmethod
    def list_teacher_event(
        cls,
        agent: TeacherAgent,
        date_range_start: datetime | None = None,
        date_range_stop: datetime | None = None,
    ) -> QuerySet[Event]:
        event_qs = Event.objects.all().filter(teacher=agent.id)
        if date_range_start is not None:
            event_qs = event_qs.filter(
                start__gt=date_range_start,
            )
    ...
    def get_or_create_event(
        ...
        **defaults,
        ...
        event = Event.objects.get_or_create(**payload, defaults=defaults)
        return event

    @classmethod
    def update_event(cls, event: Event, **values: dict) -> Event:
    ...
    @classmethod
    def add_agents(cls, event: Event, students: Iterable[StudentAgent]) -> Event:
